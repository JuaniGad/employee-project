...
const EditSalary = () => {
  ...
  const loadSalariesData = async () => {
    const result = await axios.get(`http://localhost:8080/salary/${id}`);
  ...
  const updateSalary = async (e) => {
    ...
    await axios.put(`http://localhost:8080/salary/update/${id}`, salary);
    console.log(salary);
  ...
  return (
    <div className="col-sm-8 px-5">
      <h2 className="mt-5">Edit Salary</h2>
      <form onSubmit={(e) => updateSalary(e)}>
